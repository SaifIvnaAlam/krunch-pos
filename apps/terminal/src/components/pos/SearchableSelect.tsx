...
function measurePanel(trigger: HTMLElement): PanelPos {
  ...
  const maxHeight = Math.min(280, Math.max(120, openUp ? spaceAbove - gap : spaceBelow - gap));
  return {
      ...
      {open && panel
        ? createPortal(
            <div
              ref={listRef}
              style={panelStyle}
              className="flex flex-col overflow-hidden rounded-[10px] border border-solid [border-color:var(--pos-divider)] bg-[var(--pos-card)] shadow-lg"
            >
              <div className="shrink-0 border-b border-solid [border-color:var(--pos-divider)] p-1.5">
